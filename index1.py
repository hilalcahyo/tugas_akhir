...
def preprocess(total_row: int = 10000, total_thread_processor: int = 2) -> None:
    ...
    # Crawl File Name
    file_name: str = "crawl_testing.csv"
    ...
    df.to_csv("result_preprocess_testing.csv")
...
def load_model(total_row=29000, filename: str = None):
    ...
    import seaborn as sn
    df_cf_matrix = pd.DataFrame(cf_matrix, index = [i for i in ["Negative Tweets", "Positive Tweets"]],
    columns = [i for i in ["Negative Tweets", "Positive Tweets"]])

    plt.figure(figsize = (10,7))
    sn.heatmap(cf_matrix, annot=True, cbar=False, fmt="d")
...
if __name__ == "__main__":
    ...
    #training(total_row=10000, embedding_dim=200, batch_size=64, num_epochs=3)
    load_model(total_row=2000, filename="load.csv")
